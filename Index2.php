<?php
include_once 'page3/api/dbConfig.php';
$query="SELECT * FROM classes";
$result= $con->query($query);

$query1="SELECT DISTINCT Class FROM classes";
...
<body>
	...
	<section class="main__part">
		<div class="navigation">
			<a class="link" href="index.php">Главная</a> -> Рейтинг
		</div>
		<div>
			<select id="class" name="class" class="select__classes" onChange="getStudent(this.value)">
			<option value="all">Выберите класс</option>
			<?php while ($row=$result->fetch_array(MYSQLI_ASSOC)) { ?>
			<option value=<?php echo $row['IDClass']?>><?php echo $row['Class']?> <?php echo $row['ClassLetter']?></option>
			<?php } ?>
			</select>
			<select id="class__par" name="class__par" class="select__classes" onChange="getParStudent(this.value)">
			<option value="all">Выберите параллель</option>
			<?php while ($row=$result1->fetch_array(MYSQLI_ASSOC)) { ?>
			<option value=<?php echo $row['Class']?>><?php echo $row['Class']?></option>
			<?php } ?>
			</select>
			<button class="allclasses" onclick="getAllStudent(this.value)">Показать все классы</button>
		</div>
